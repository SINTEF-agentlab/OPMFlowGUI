...
class _CpuCoreBar(QWidget):
    ...
    def __init__(self, core_index: int, parent: QWidget | None = None) -> None:
        ...
        self._bar.setStyleSheet(
            f"QProgressBar {{ background-color: {BG_TERTIARY}; border: none;"
            f" border-radius: 5px; }}"
...
class SystemMonitorPanel(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        # ── Per-core CPU bars ────────────────────────────────────────────
        cpu_section = QWidget()
        cpu_section.setStyleSheet(
            f"QWidget {{ background-color: {BG_SECONDARY}; border: 1px solid {BORDER};"
            f" border-radius: 8px; }}"
        )
        cpu_outer = QVBoxLayout(cpu_section)
        cpu_outer.setContentsMargins(12, 10, 12, 10)
        cpu_outer.setSpacing(6)

        cpu_title = QLabel("Per-Core CPU Utilisation")
        cpu_title.setStyleSheet(
            f"color: {ACCENT_LIGHT}; font-size: 13px; font-weight: bold;"
            " background: transparent; border: none;"
        )
        cpu_outer.addWidget(cpu_title)
        ...
        root.addWidget(cpu_section, 1)

        # ── Flow processes table ─────────────────────────────────────────
        proc_section = QWidget()
        proc_section.setStyleSheet(
            f"QWidget {{ background-color: {BG_SECONDARY}; border: 1px solid {BORDER};"
            f" border-radius: 8px; }}"
        )
        proc_outer = QVBoxLayout(proc_section)
        proc_outer.setContentsMargins(12, 10, 12, 10)
        proc_outer.setSpacing(6)

        proc_title = QLabel("Running OPM Flow Processes")
        proc_title.setStyleSheet(
            f"color: {ACCENT_LIGHT}; font-size: 13px; font-weight: bold;"
            " background: transparent; border: none;"
        )
        proc_outer.addWidget(proc_title)
        ...
        self._proc_table.setStyleSheet(
            f"QTableWidget {{ background-color: {BG_PRIMARY}; border: none;"
    ...
    def _update_processes(self) -> None:
        ...
        self._card_flow_count.set_value(
            str(count),
            ACCENT if count > 0 else TEXT_MUTED,
        )
